...
function NewEpisodes({ endpoint }) {
    ...
    return (
        <div className="px-5 lg:px-12 mb-14">
            <div className="flex justify-between items-start">
                <h1 className="mb-4 text-lg lg:text-3xl font-bold flex items-center gap-2"><SquarePlay size={30} />New Episodes</h1>
                <p className="mb-2 text-xs lg:text-base font-medium text-gray-400 flex items-center gap-2">VIEW RELEASE CALENDER <ChevronRight color="gray" size={22} /></p>
            </div>
            <h1 className="mb-3 text-2xl">Today</h1>
            <hr className="border-gray-500 mb-4" />
            <div className="lg:grid grid-cols-3 gap-4 w-full cursor-pointer mb-3">
                {movies.length && movies.map((movie, index) => (
                    <NewEpisodesCard item={movie} index={index} />
                ))}
            </div>
            {isMore && (
                <div className="lg:grid grid-cols-3 gap-4 w-full cursor-pointer">
